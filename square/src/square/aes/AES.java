//cloned from https://github.com/adamchiang/AES
package square.aes;
...
public class AES {
    ...
    private Data[] roundKey_ = null;
    /* Data object to hold state_ */
    private Data state_ = null;
    /* Data object to hold the input key */
    private Data key_ = null;

    public AES(Data key, int rounds)
    {
        key_ = key;
        rounds_ = rounds;
        expandKey();
     ...
     */
    private static void subBytes(Data state, boolean encryptFlag) {
     ...
     */
    public static void mixColumns(Data state, boolean encryptFlag) {
    ...
    private void addRoundKey(int currentRound) {
        state_.XOR(roundKey_[currentRound]);
    }
    ...
    private void expandKey() {
        Data expandedKey_ = new Data((16 * (rounds_ + 1)), key_);
        int offset = key_.getNumCols();

        for (int col = offset; col < expandedKey_.getNumCols(); col++) {
            if (col % offset == 0) {
            /* Rotate and XOR with rCon */
                byte tempByte = expandedKey_.byteMatrix[0][col - 1];
                for (int row = 0; row < expandedKey_.getNumRows(); row++) {
                    expandedKey_.byteMatrix[row][col] = expandedKey_.byteMatrix[row][col - offset];
                    byte temp = row == 3 ? tempByte : expandedKey_.byteMatrix[row + 1][col - 1];
                    expandedKey_.byteMatrix[row][col] ^= subByte(temp, true);
                }
                expandedKey_.byteMatrix[0][col] ^= rCon[col / offset];
            } else if (BITS == 256 && col % key_.getNumRows() == 0) {
                for (int row = 0; row < expandedKey_.getNumRows(); row++) {
                    expandedKey_.byteMatrix[row][col] = subByte(expandedKey_.byteMatrix[row][col - 1], true);
                    expandedKey_.byteMatrix[row][col] ^= expandedKey_.byteMatrix[row][col - offset];
                }
            } else {
                for (int row = 0; row < expandedKey_.getNumRows(); row++) {
                    expandedKey_.byteMatrix[row][col] = expandedKey_.byteMatrix[row][col - 1];
                    expandedKey_.byteMatrix[row][col] ^= expandedKey_.byteMatrix[row][col - offset];
                }
            }
        }
        roundKey_ = expandedKey_.toDataArray();
    }

    /**
     * Looks up byte to substitute in sBox/invSBox
     */
    private static byte subByte(byte b, Boolean useSBox) {
        int x = (b >> 4) & 0x0F;
        int y = b & 0x0F;
     ...
     * AES encryption
     */
    public Data encrypt(byte[] plainState) {
        state_ = new Data(plainState);
        if (rounds_ == 0)
            return new Data(state_.byteArray);
        int currentRound = 0;
        addRoundKey(currentRound);
        while (currentRound < rounds_ - 1)
        {
            currentRound++;
            subBytes(state_,true);
            shiftRows(state_, true);
            mixColumns(state_, true);
            addRoundKey(currentRound);
        }
        //TODO remove the mix columns step in the last operation for testing purpose
        if (currentRound == 3)
        {
            currentRound++;
            subBytes(state_,true);
    ...
    public LambdaSet encrypt(LambdaSet lambdaSet)
    {
        List<Data> dataSet = new ArrayList<>(lambdaSet.internalSet_);
        return new LambdaSet(dataSet.stream().map(d -> encrypt(d.byteArray)).collect(Collectors.toList()));
    }

    /**
     * AES decrpytion
     */
    public static Data decryptOneRound(byte[] cipher, Data roundKey, int roundsUsedToEncrypt){
         /* Read in key_ */
        Data state = new Data(cipher);

        state.XOR(roundKey);
        //mixColumns(state, false);
        shiftRows(state, false);
        subBytes(state, false);
        return new Data(state.byteArray);
    }

    public static LambdaSet decryptOneRound(LambdaSet lambdaSet, Data roundKey, int roundsUsedToEncrypt)
    {
        List<Data> dataSet = new ArrayList<>(lambdaSet.internalSet_);
        return new LambdaSet(dataSet.stream().map(d -> decryptOneRound(d.byteArray, roundKey, roundsUsedToEncrypt)).collect(Collectors.toList()));
    }


    /* XOR constants for key_ expansion */
    ...
    private static final int[] SHIFT_ENCRYPT = {0, -1, -2, -3};
    /* Integer array to gold values to undo shift */
    private static final int[] SHIFT_DECRYPT = {0, 1, 2, 3};

    public Data getRoundKey(int round)
    {
        return roundKey_[round];
    }
}
